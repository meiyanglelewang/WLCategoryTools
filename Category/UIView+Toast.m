 ...
 *  @param duration 持续时间
 *  @param position    展示位置
 *  @param bgClolor    框框背景色
 *  @param textColor    字体颜色
 */
- (void)wl_makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position backgroundColor:(UIColor *)bgClolor messageTextColor:(UIColor *)textColor{
    UIView *toast = [self wl_viewForMessage:message title:nil image:nil backgroundColor:bgClolor messageTextColor:textColor];
...
- (void)showToast:(UIView *)toast tapCallback:(void(^)(void))tapCallback{
    if (mShowView) {
        [mShowView removeFromSuperview];
    }
    mShowView = toast;
    toast.center = [self centerPointForPosition:CSToastPositionCenter withToast:toast];
...
{
    if (mShowView) {
        if ([mTimer isValid]) {[mTimer invalidate];}
        [mShowView removeFromSuperview];
    }
    mShowView = toast;
...
- (UIView *)wl_viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image backgroundColor:(UIColor *)bgClolor messageTextColor:(UIColor *)textColor{
    // sanity
    if((message == nil) && (title == nil) && (image == nil) && (bgClolor == nil) && (textColor == nil)) return nil;
    ...
    if (CSToastDisplayShadow) {
        wrapperView.layer.shadowColor = bgClolor.CGColor;
        wrapperView.layer.shadowOpacity = CSToastShadowOpacity;
        wrapperView.layer.shadowRadius = CSToastShadowRadius;
        wrapperView.layer.shadowOffset = CSToastShadowOffset;
    }
    
    wrapperView.backgroundColor = [bgClolor colorWithAlphaComponent:CSToastOpacity];
